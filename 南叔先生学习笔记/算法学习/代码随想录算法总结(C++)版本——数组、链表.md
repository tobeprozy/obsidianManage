...
![[Pasted image 20220407215517.png]]
```c++
class Solution {
public:
...
- ![[Pasted image 20220407220039.png]]
下面采用的设置一个虚拟头结点：
```c++
class MyLinkedList {
public:
...
![[Pasted image 20220407220318.png]]
首先定义一个cur指针，指向头结点，再定义一个pre指针，初始化为null。

然后就要开始反转了，首先要把 cur->next 节点用tmp指针保存一下，也就是保存一下这个节点。
![[Pasted image 20220407220323.png]]
#### 双指针法
```c++
class Solution {
public:
...
图示两个链表在节点 c1 开始相交：
![[Pasted image 20220407220924.png]]

题目数据 保证 整个链式结构中不存在环。

注意，函数返回结果后，链表必须 保持其原始结构 。
示例 1：
![[Pasted image 20220407220949.png]]
